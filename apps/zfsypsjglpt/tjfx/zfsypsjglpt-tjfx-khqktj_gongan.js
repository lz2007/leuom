...
avalon.component(name, {
    ...
    defaults: {
        ...
        breadcrumbClick(index, item, list) {
            ...
            if (0 === index) {
                let orgObj = storage.getItem('tjfx-khqktj-breadcrumb-obj_gongan');
        ...
        exportBtn() {
            ...
            switch (this.currentTable) {
                case 'main':
                    baseUrl = '/gmvcs/stat/l/eva/complete/info/ga/exportByUser';
                    break;
                case 'jdqk':
                ...
                case 'scqk':
                    ...
                    break;
                case 'glqk':
                    baseUrl = '/gmvcs/stat/l/eva/complete/match/info/ga/exportByExcel';
                    break;
                case 'ccqk':
                    baseUrl = '/gmvcs/stat/l/eva/complete/spotCheck/info/ga/exportByExcel';
                    break;
            }
        ...
        get_table_list(item, type, breadcrumbMainClick) {
            ...
            switch (this.currentTable) {
                case 'main':
                    url = '/gmvcs/stat/l/eva/complete/info/ga';
                    break;
                case 'jdqk':
                ...
                case 'scqk':
                    ...
                    break;
                case 'glqk':
                    url = '/gmvcs/stat/l/eva/complete/match/info/ga';
                    break;
                case 'ccqk':
                    url = '/gmvcs/stat/l/eva/complete/spotCheck/info/ga';
                    break;
            }
            ...
            ajax({
                ...
                if (result.code == 0) {
                    ...
                    if (type && ret_data.length > 0) {
                        ...
                        let storageData = storage.getItem('zfsypsjglpt-tjfx-khqktj_gongan');
                        this.list = storageData.tableList;
                    ...
                    if ('main' == this.currentTable) {
                        if(this.isSearch) {
                            let breadcrumbObj = storage.getItem('tjfx-khqktj-breadcrumb-obj_gongan');
                            this.breadcrumbList = [
                                {
                                    orgName: '首页',
                        ...
                        storage.setItem('zfsypsjglpt-tjfx-khqktj_gongan', storageData, 0.5);
                    } else {
                        storage.setItem('zfsypsjglpt-tjfx-khqktj_gongan-table-detail', storageData, 0.5);
                    }
                    let tableObj =  {};
                    tableObj.showType = this.currentTable;
                    storage.setItem('tjfx-khqktj-table-obj_gongan', tableObj, 0.5);
                }
            });
        },
        inintTableDrag() {
        ...
        config() {
            if ('main' == this.currentTable) {
                ...
                let tableObj = {};
                tableObj.showType = this.currentTable;
                storage.setItem('tjfx-khqktj-table-obj_gongan', tableObj);
            }
        },
        getConfigInfo() {
            ajax({
                // url: '/api/jj-qzpz',
                url: '/gmvcs/stat/ge/config/info/ga',
                method: 'get',
                data: {}
            }).then(result => {
                if (result.code != 0) {
        ...
        tableConfig_01: {
            ...
            name: 'tjfx-khqktj-table_01_gongan',
            arr: ['khqktj-list-header', 'khqktj-list-header-upper']
        },
        tableConfig_02: {
            body: 'khqktj-list-content',
            name: 'tjfx-khqktj-table_02_gongan',
            arr: ['khqktj-list-header', 'khqktj-list-header-upper']
        },
        onInit(e) {
            zhkp_vm = this;
            ...
            ajax({
                ...
                let getFromStorage = !!storage.getItem('zfsypsjglpt-tjfx-khqktj_gongan'); // getFromStorage 判断是否需要初始化页面；false为重新从后台拿数据初始化，true为从Storage拿数据填充
                let storageData = storage.getItem('zfsypsjglpt-tjfx-khqktj_gongan');
                ...
                if (!getFromStorage) {
                    ...
                    storage.setItem('tjfx-khqktj-breadcrumb-obj_gongan', orgObj, 0.5);
                } else {

                    if (!!window.hadSaveKhqktjConf) { // hadSaveConf 是否保存权重配置数据，true为重新更新数据，false时直接取localstorage的数据
                        // 修改权重配置后重置查询条件等数据
                        storage.removeItem('zfsypsjglpt-tjfx-khqktj_gongan');
                        ...
                        let tableObj = storage.getItem('tjfx-khqktj-table-obj_gongan');
                        ...
                        if ('main' != tableObj.showType) {
                            storageData = storage.getItem('zfsypsjglpt-tjfx-khqktj_gongan-table-detail');
                        } else {
            ...
            menuServer.menu.then(menu => {
                ...
                if (this.isPolice) {
                    // 警员屏蔽所属部门和统计对象
                    $(".zfsypsjglpt_tjfx_khqktj").find(".search_box").find(".tips-img").css({
                        "right": "400px",
                        "top": "14px"
                    });
                ...
                if (false == _this.authority.SEARCH)
                    $(".zfsypsjglpt_tjfx_khqktj_jj_table").css("top", "54px");
                if (!_this.authority.SEARCH && !_this.authority.QZPZ && !_this.authority.EXPORT)
...
function windowResize() {
    ...
    if (v_height > 740) {
        ...
        $(".zfsypsjglpt_tjfx_khqktj").height(740);
...
function listenTableScrollHeight() {
    let $this = $(".zfsypsjglpt_tjfx_khqktj .khqktj-list-content");
    let clientHeight = $this[0].clientHeight;
    let scrollHeight = $this[0].scrollHeight;
    if (scrollHeight > clientHeight) {
        zhkp_vm.showScroll = true;
        $(".zfsypsjglpt_tjfx_khqktj .khqktj-list-header-upper").css({
            "padding-right": "17px"
        });
        $(".zfsypsjglpt_tjfx_khqktj .khqktj-list-header").css({
            "padding-right": "17px"
        });
    } else {
