...
export const name = 'zfsypsjglpt-jdkp-jycx-ykp_jiaojing';
...
function Tools() {
    ...
    this.get_jycx_info = function (param) {
        ajax({
            url: '/gmvcs/audio/violation/search/' + window.kpcczl_jycxkp,
            ...
            if (ret.code == 0) {
                ...
                kp_hccl.checkLooking = (ret.data.wfxwmc || '-') + '(' + (ret.data.jdsbh || '-') + ')';
    ...
    this.get_eva_info = function () {
        ajax({
            url: '/gmvcs/audio/violation/eva/info/' + window.kpcczl_jycxkp,
    ...
    this.set_review_info = function (params) {
            ajax({
                url: '/gmvcs/audio/violation/review',
        ...
        this.get_review_info = function () {
            ajax({
                url: '/gmvcs/audio/violation/review/info/' + window.kpcczl_jycxkp,
...
avalon.component(name, {
    template: __inline('./zfsypsjglpt-jdkp-jycx-ykp_jiaojing.html'),
    defaults: {
        back() {
            $("#gm_webplayer").hide();
            delete_ocx();
        ...
        onInit(event) {
            ...
            menuServer.menu.then(menu => {
                ...
                avalon.each(func_list, function (k, v) {
                    switch (v) {
                        case "AUDIO_FUNCTION_JDKP_JTWFKP_JYCX_HCCL":
