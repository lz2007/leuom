...
require('./zfsypsjglpt-zfda-ajgl-ck-new.css');
export const name = 'zfsypsjglpt-zfda-ajgl-ck-new';

let ajgl_ck_Man = null;
let ajglMan = null;
avalon.component(name, {
    template: __inline('./zfsypsjglpt-zfda-ajgl-ck-new.html'),
    defaults: {
        authority: { // 按钮权限标识
        ...
        //关联媒体 -- 接警
        jq_clickClass: 'glmt_jj_new', //按钮样式
        jq_data: [], //展示的关联媒体
        ...
        toggleMt(e) {
            ...
            if (/wzk/img.test($(e.target).attr('src'))) {
                $(e.target).attr('src', '/static/image/zfda-ajgl-ck/zk.png?__sprite');
            } else {
                $(e.target).attr('src', '/static/image/zfda-ajgl-ck/wzk.png?__sprite');
            };
            var aim = this[this.glmt_mark + '_data'][$(e.target).attr('pos')];
        ...
        playFile(e) {
            ...
            var rid = e.currentTarget.children[0].children[3].attributes.name.nodeValue;
            Tools.checker.checkedWork(rid);
            var type = e.currentTarget.children[0].children[3].attributes.type.nodeValue;
            ...
            ajax({
                ...
                if (ret.code == 0) {
                    ...
                    ret.data.keyFile = ret.data.match == true ? '是' : '否';
    ...
    searchMediaByJqbh: function () {
        ...
        ajax({
            ...
            if (ret.code == 0) {
                ...
                ret.data.zbmjxm = ret.data.zbmjxm.join(',') + '(' + ret.data.zbmj.join(',') + ')';
                ...
                ['preBaseFiles', 'handelerBaseFiles'].forEach(function (val, key) {
                    ret.data[val].forEach(function (val, key) {
                        ...
                        val.toggle = false;
                        val.keyFile = val.match == true ? '是' : '否';
                        val.type = (function () {
                            switch (val.type) {
