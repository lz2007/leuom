 ...
 * @LastEditTime: 2019-01-09 17:36:02
...
export const name = "jdzxpt-sjcc-aj";
import "./jdzxpt-sjcc-aj.less";
...
avalon.component(name, {
    ...
    defaults: {
        ...
        onInit(e) {
            aj_vm = e.vmodel;
            this.resetDatepicker(); //重置时间选择组件
            this.tree_init = false;
            let storage_item = storage.getItem('jdzxpt-sjcc-aj');
            ...
            //         aj_time_range.time_range[0] = storage_item.range_flag.toString();
            //     }
            //     aj_startTime_vm.aj_startTime = storage_item.ajax_data.startTime;
            //     aj_endTime_vm.aj_endTime = storage_item.ajax_data.endTime;

            //     this.included_status = storage_item.ajax_data.includeChild;
            //     this.generateNum = storage_item.ajax_data.generateNum || ""; //生成条数
            //     this.generateNum_title = this.generateNum || aj_vm.aj_txt.supportPositiveIntegersAnd0;
        ...
        onReady() {
            ...
            menuServer.menu.then(menu => {
                ...
                avalon.each(list, function (index, el) {
                    if (/^EVA_FUNCTION_JDKP_SJCC_AJKP/.test(el))
                        avalon.Array.ensure(func_list, el);
                });
                avalon.each(func_list, function (k, v) {
                    switch (v) {
                        case "EVA_FUNCTION_JDKP_SJCC_AJKP_SEARCH":
        ...
        ajaxDepTree(storage_item) {
            ...
            ajax({
                ...
                aj_tree_vm.tree_data = deptemp;
                if (storage_item) {
                    aj_tree_vm.tree_code = storage_item.ajax_data.orgPath;
                    aj_tree_vm.tree_key = storage_item.tree_key;
                    aj_tree_vm.tree_title = storage_item.tree_title;
                } else {
                    aj_tree_vm.tree_code = deptemp[0].path;
                    aj_tree_vm.tree_key = deptemp[0].key;
                    aj_tree_vm.tree_title = deptemp[0].title;
                }
        ...
        ajaxIsCreated() {
            ajax({
                url: '/gmvcs/audio/case/random/result/empty',
                method: 'get',
                data: {}
            }).then(result => {
                aj_vm.isCreated = (result.data == true ? 1 : 2);
            });
        },
        //部门树下拉框是否包含子部门
        clickBranchBack(e) {
        ...
        //获取表格数据
        getTableList() {
            this.startTime = aj_startTime_vm.aj_startTime;
            this.endTime = aj_endTime_vm.aj_endTime;
            let ajax_data = {
                "endTime": getTimeByDateStr(this.endTime, true),
                "includeChild": this.included_status,
                "orgPath": aj_tree_vm.curTree || aj_tree_vm.tree_code,
                "page": 0,
            ...
            ajax_data.orgName = aj_tree_vm.tree_title; //用于后台下次返回已生成的显示部门
            ajax_data.generateNum = -1; //只有当可生成并且输入框有值的时候，才不传-1值
            if (aj_vm.isCreated == 1 && aj_vm.generateNum) { //可生成的时候且输入框有值的时候
                ajax_data.generateNum = $.trim(aj_vm.generateNum);
            }
            if (aj_vm.generateNum > 100) {
        ...
        ajaxTableData(ajax_data) {
            ajax({
                url: '/gmvcs/audio/case/random/generate',
                method: 'post',
                data: ajax_data
            }).then(result => {
                if (result.code != 0) {
                ...
                if (!result.data.pageObject.overLimit && result.data.pageObject.totalElements == 0) {
                    ...
                    let storage_item = {
                        "ajax_data": temp_data,
                        "total": 0,
                        "tree_key": aj_tree_vm.tree_key,
                        "tree_title": aj_tree_vm.tree_title
                    };
                    storage.setItem('jdzxpt-sjcc-aj', storage_item, 0.5);
                    return;
                }
                if (!this.change_page) {
                ...
                avalon.each(result.data.pageObject.currentElements, function (index, item) {
                    ...
                    //     ajax({
                    //         url: `/gmvcs/uap/org/getFullName?orgCode=${item.data.sldw}&prefixLevel=${prefixLevel}&separator=${separator}`,
                    //         method: 'get'
                    //     }).then(result => {
                    //         ret_data[index].sldw = result.data;
                    //     });
                    // }
                    ret_data[index].table_index = temp + index; //行序号
                    ret_data[index].name_id = (item.data.zbmjxm || "-") + "(" + (item.data.zbr || "-") + ")"; //姓名（警号）
                    ret_data[index].afsj = moment(item.data.afsj).format("YYYY-MM-DD HH:mm:ss"); //案发时间
                    ...
                    //涉案人员列表
                    ret_data[index].involvedPeoples = ''; //先赋值，相当于声明了该变量，防止undefined的出现
                    if (item.data.involvedPeoples) {
                        item.data.involvedPeoples.forEach((val, i, val_arr) => {
                            if (i != val_arr.length - 1) {
                                ret_data[index].involvedPeoples += (val.rymc + ","); //将人员名字用逗号拼接起来
                            } else {
                                ret_data[index].involvedPeoples += val.rymc;
                            }
                        });
                    }
                    //警情编号列表
                    if (item.data.policeSituation) {
                        item.data.policeSituation.forEach((val, i, val_arr) => {
                            if (i != val_arr.length - 1) {
                                ret_data[index].policeSituation += (val + ","); //将警情编号用逗号拼接起来
                            } else {
                                ret_data[index].policeSituation += val;
                            }
                        });
                    }
                });
                ...
                aj_vm.depTreeTitle = result.data.criteria.orgName;
                aj_vm.generateNum = result.data.criteria.generateNum;
                aj_vm.startTime = moment(result.data.criteria.startTime).format("YYYY-MM-DD");
                aj_vm.endTime = moment(result.data.criteria.endTime).format("YYYY-MM-DD");
                //设置缓存
                let storage_item = {
                    "ajax_data": temp_data,
            ...
            switch (val) {
                case 'generateNum':
                    if (e.target.value != "") {
                        ...
                        this.generateNum_title = aj_vm.aj_txt.supportPositiveIntegersAnd0;
                    }
                    break;
            }
            if (e.keyCode == "13") {
        ...
        //重置时间组件,默认时间是本周
        resetDatepicker() {
            aj_time_range.time_range[0] = "0";
            aj_time_range.range_flag = 0;
...
let aj_endTime_vm = avalon.define({
    ...
    handlerChange(e) {
        this.aj_endTime = e.target.value;
    },
});
//获取语言对象
function getLan() {
