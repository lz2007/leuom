...
export const name = "zfsypsjglpt-sypgl-baqmt-detail";
require("./zfsypsjglpt-sypgl-baqmt-detail.css");
...
avalon.component(name, {
    template: __inline("./zfsypsjglpt-sypgl-baqmt-detail.html"),
    defaults: {
        ...
        edit_save: true,
        remarkState: true, //备注信息   true -- 可编辑状态，false -- 不可编辑状态
        ...
        prevClick() {
            ...
            let left = $(".zfsypsjglpt_sypgl_baqmt_detail .glyw_content .result_list").position().left + (this.showNumber * 376);
            $(".zfsypsjglpt_sypgl_baqmt_detail .glyw_content .result_list").css({
                left: left
            });
            ...
            if (left == 0) {
                ...
                this.prevStopClass = true;
                setTimeout(() => {
                    baqmt_detail_vm.prevStopClass = false;
                }, 550);
            }
        },
        nextClick() {
            ...
            if ($(".zfsypsjglpt_sypgl_baqmt_detail .glyw_content .result_list").width() - Math.abs(left) <= $(".zfsypsjglpt_sypgl_baqmt_detail .glyw_content .result_list_panel").width()) {
                this.nextStopClass = true;
            } else {
                this.nextStopClass = true;
                setTimeout(() => {
                    baqmt_detail_vm.nextStopClass = false;
                }, 550);
            }
        },
        ...
        opt_baqmt_detail: avalon.define({
            $id: "opt_baqmt_detail",
            authority: { // 按钮权限标识
                "MARK": false, // 备注信息
                "BZXX": false, // 标注信息
        ...
        onInit(e) {
            ...
            this.record_item = storage.getItem("zfsypsjglpt-sypgl-baqmt-detail");
            this.block_num = 0;
            ...
            menuServer.menu.then(menu => {
                ...
                avalon.each(func_list, function (k, v) {
                    switch (v) {
                        case "AUDIO_FUNCTION_SYPGL_ZFYSYP_BAQMT_MARK":
                            _this.opt_baqmt_detail.authority.MARK = true;
                            break;
                        case "AUDIO_FUNCTION_SYPGL_ZFYSYP_BAQMT_BZXX":
                            _this.opt_baqmt_detail.authority.BZXX = true;
                            break;
                        case "AUDIO_FUNCTION_SYPGL_ZFYSYP_BAQMT_DOWNLOAD":
                            _this.opt_baqmt_detail.authority.DOWNLOAD = true;
                            break;
                        case "AUDIO_FUNCTION_SYPGL_ZFYSYP_BAQMT_SCGL":
                            _this.opt_baqmt_detail.authority.SCGL = true;
                            break;
                        case "AUDIO_FUNCTION_SYPGL_ZFYSYP_BAQMT_TJGL":
                            _this.opt_baqmt_detail.authority.TJGL = true;
                            break;
                    }
                });
            }).then(() => {
                if (_this.opt_baqmt_detail.authority.MARK == false && _this.opt_baqmt_detail.authority.BZXX == false) {

                } else if (_this.opt_baqmt_detail.authority.MARK == false) {
                    $(".zfsypsjglpt_sypgl_baqmt_detail .relation_info .relation_list").height(610);
                } else if (_this.opt_baqmt_detail.authority.BZXX == false) {
                    $(".zfsypsjglpt_sypgl_baqmt_detail .zfjlysyp-detail-content").height(610);
                }
            });
        ...
        getMark() { //获取备注信息
            ajax({
                url: `/gmvcs/baq/caseareafile/queryMark?rid=${this.record_item.rid}`,
                // url: `/api/queryMark?rid=${this.record_item.rid}`,
                method: 'get',
                data: {}
            }).then(result => {
                if (result.code != 0) {
                ...
                if (result.code == 0) {
                    if (result.data && result.data.markTime) {
                        this.remarkContent = result.data.mark;
                        this.remarkPeople = `${result.data.markUserName}(${result.data.markUserCode}) 备注于 ${moment(parseInt(result.data.markTime)).format("YYYY-MM-DD HH:mm:ss")}`;
        ...
        onReady() {
            ...
            $(window).bind('beforeunload', function (event) { //ie
                ...
                if (global.location.hash.indexOf("zfsypsjglpt-sypgl-baqmt-detail") > -1)
                    avalon.history.setHash("/zfsypsjglpt-sypgl-baqmt");
            });
            ...
            this.$watch("edit_save", (v) => {
                if (v) {
                    $(".zfsypsjglpt_sypgl_baqmt_detail .relation_tag").addClass("inputState");
                } else {
                    $(".zfsypsjglpt_sypgl_baqmt_detail .relation_tag").removeClass("inputState");
                    setChildType(tag_type_vm.tag_type_jj[0], tag_type_vm.child_typeCode);
                }
            });
            this.$fire('edit_save', this.edit_save);

            this.$watch("remarkState", (v) => {
                if (v) {
                    $(".zfsypsjglpt_sypgl_baqmt_detail .remark_info").removeClass("inputState");
                } else {
                    $(".zfsypsjglpt_sypgl_baqmt_detail .remark_info").addClass("inputState");
                }
            });
            this.$fire('remarkState', this.remarkState);
        },
        onDispose() {
            ...
            baqmt_detail_vm = null;
            storage.removeItem("zfsypsjglpt-sypgl-baqmt-detail");
        },
        returnBtn() {
            delete_ocx();
                ...
                ajax({
                    ...
                    if (result.code == 0) {
                        ...
                        if ($("#iframe_others").is(":visible")) {
                            if (baqmt_detail_vm.mediaInfoShow) {
                                setTimeout(function () {
                                    $("#iframe_others").css({
                                ...
                                setTimeout(function () {
                                    baqmt_detail_vm.dialog_status = false;
                                }, 300);
                            }
                        }
                    }
        ...
        tag_btn() { //显示标注信息
            ...
            let width1 = ($(".mark_info").width() && $(".mark_info").width() > 0) ? $(".mark_info").width() : $(".remark_info").width();
            $(".zfyps_area").width(width1 - 114);
            ...
            $(window).resize(function () {
                ...
                let width2 = ($(".mark_info").width() && $(".mark_info").width() > 0) ? $(".mark_info").width() : $(".remark_info").width();
                $(".zfyps_area").width(width2 - 114);
            });
        ...
        save_btn() { //保存标记
            ...
            let childLabelTypeId = tag_type_son_vm.curTag || tag_type_son_vm.tag_type_jj[0];
            ajax({
        ...
        remarkSave() {
            ...
            let markTime = moment();
            ajax({
                url: `/gmvcs/baq/caseareafile/saveMark`,
                // url: `/api/saveMark`,
                method: 'post',
                data: {
                    rid: this.record_item.rid,
                    mark: this.remarkContent,
                    markUserCode: storage.getItem("userCode"),
                    markUserName: storage.getItem("userName"),
                    markTime: parseInt(markTime.format("x"))
                }
            }).then(result => {
                if (result.code != 0) {
                    notification.error({
                ...
                this.getMark();
                // this.remarkPeople = `${storage.getItem("userName")}(${storage.getItem("userCode")}) 备注于 ${markTime.format("YYYY-MM-DD HH:mm:ss")}`;
            });
        },
...
function getFileByRid(flag) {
    ajax({
        ...
        url: '/gmvcs/audio/basefile/findBaseFileByRid?rid=' + baqmt_detail_vm.record_item.rid,
        method: 'get',
        data: {}
    }).then(result => {
        if (result.code != 0) {
        ...
        media_temp.orgName = baqmt_detail_vm.record_item.orgName || "暂无"; //执勤部门
        ...
        baqmt_detail_vm.fileName = media_temp.fileName;
        ...
        if (result.data.saveTime == "-1")
            ...
            baqmt_detail_vm.show_timeout = true;
            baqmt_detail_vm.show_GMPlayer = false;
            baqmt_detail_vm.show_img = false;
            baqmt_detail_vm.show_other = false;

            media_temp.saveTime = "已过期";
            media_temp.saveSite = "-";
            baqmt_detail_vm.unClick_media = true;
            ...
            if (flag) {
                ...
                baqmt_detail_vm.zfyps_dialog_width = 300;
                baqmt_detail_vm.zfyps_dialog_height = 178;

                baqmt_detail_vm.cancelText = "关闭";
                baqmt_detail_vm.zfyps_dialog_show = true;
                baqmt_detail_vm.dialog_status = false;
        ...
        baqmt_detail_vm.media_info = media_temp;
        ...
        avalon.each(result.data.surveil, function (index, item) {
            ...
            result_arr[result_len].ajlx = "-"; //案件类型
        ...
        avalon.each(result.data.violation, function (index, item) {
            ...
            result_arr[result_len].happenTime = formatDate(item.wfsj); //发生时间
            result_arr[result_len].ajlx = "-"; //案件类型
            result_arr[result_len].place = item.wfxwmc; //事故地点
        ...
        avalon.each(result.data.force, function (index, item) {
            ...
            result_arr[result_len].people = item.userName + "(" + item.userCode + ")"; //执勤民警
            result_arr[result_len].happenTime = formatDate(item.wfsj); //发生时间
            result_arr[result_len].ajlx = item.wfxwmc[0] || "-"; //案件类型
            result_arr[result_len].place = item.wfdz; //事故地点
            result_arr[result_len].key = item.xh; //主键
        ...
        avalon.each(result.data.accident, function (index, item) {
            ...
            result_arr[result_len].ajlx = item.wfxwmc || "-"; //案件类型
        ...
        avalon.each(result.data.caseMains, function (index, item) {
            ...
            result_arr[result_len].ajlx = item.ajlbmc || "-"; //案件类型
            result_arr[result_len].place = item.afdd; //事故地点
        ...
        avalon.each(result.data.policeSituationResponses, function (index, item) {
            ...
            result_arr[result_len].ajlx = item.jqlbmc || "-"; //案件类型
        ...
        $(".zfsypsjglpt_sypgl_baqmt_detail .glyw_content .result_list").width(result_arr.length * 376);
        $(".zfsypsjglpt_sypgl_baqmt_detail .glyw_content .result_list").css({
            "left": 0
        });

        baqmt_detail_vm.prevStopClass = true;
        baqmt_detail_vm.result_list = result_arr;
        if (baqmt_detail_vm.result_list.length == 0) {
            baqmt_detail_vm.nextStopClass = true;
        }
        setTimeout(() => {
            windowResize();
        }, 688);

        // if (baqmt_detail_vm.tag_toggle) //防止接口请求过慢出现的bug
        //     baqmt_detail_vm.tag_btn();

        $(".zfsypsjglpt_sypgl_baqmt_detail .relation_list .relation_li_border").hide();
        $(".zfsypsjglpt_sypgl_baqmt_detail .relation_list .relation_li_border:eq(0)").show();
        _popover();
    });
}
...
let relation_info_jj = avalon.define({
    ...
    relation_list_click(e) {
        baqmt_detail_vm.block_num = e;
        $(".zfsypsjglpt_sypgl_baqmt_detail .relation_list .relation_li_border").hide();
        $(".zfsypsjglpt_sypgl_baqmt_detail .relation_list .relation_li_border:eq(" + e + ")").show();
    },
    add_rel() {
        zfyps_detail_add_dialog.police_check = "";
        zfyps_detail_add_dialog.handle_num = "";
        baqmt_detail_vm.zfyps_dialog_width = 1100;
        baqmt_detail_vm.zfyps_dialog_height = 628;
        ...
        baqmt_detail_vm.add_rel_dialog = true;
        baqmt_detail_vm.dialog_status = false;
        baqmt_detail_vm.is_play = false;
    ...
    delete_rel(jcglArr) {
        ...
        post_data.rid = baqmt_detail_vm.record_item.rid;
...
function windowResize() {
    if (baqmt_detail_vm.result_list.length > 0) {
        if (($(".zfsypsjglpt_sypgl_baqmt_detail .glyw_content .result_list_panel").width() >= baqmt_detail_vm.result_list.length * 376) || ($(".zfsypsjglpt_sypgl_baqmt_detail .glyw_content .result_list_panel").width() >= ($(".zfsypsjglpt_sypgl_baqmt_detail .glyw_content .result_list_panel .result_list").width() - Math.abs($(".zfsypsjglpt_sypgl_baqmt_detail .glyw_content .result_list_panel .result_list").position().left)))) {
            baqmt_detail_vm.nextStopClass = true;
        } else {
            baqmt_detail_vm.nextStopClass = false;
        }
    ...
    baqmt_detail_vm.showNumber = Math.floor($(".zfsypsjglpt_sypgl_baqmt_detail .glyw_content .result_list_panel").width() / 376);

    if ($("#iframe_zfsyps").is(":visible")) {
        if (baqmt_detail_vm.dayShow) {
            setTimeout(function () {
                $("#iframe_zfsyps").css({
                    "left": ($('.ccts').position().left + 255) + "px",
                    "top": ($('.ccts').position().top + 2) + "px",
            ...
            setTimeout(() => {
                $("#iframe_zfsyps").css({
                    width: baqmt_detail_vm.zfyps_dialog_width,
                    height: baqmt_detail_vm.zfyps_dialog_height
                });
            }, 0);
        }
    }
...
let zfyps_detail_add_dialog = avalon.define({
    ...
    get_table_list() {
        ...
        let ajax_data = {
            "rid": baqmt_detail_vm.record_item.rid,
            "page": this.curPage - 1,
    ...
    add_rel_btn() {
        ...
        baqmt_detail_vm.add_rel_dialog = false;
        baqmt_detail_vm.dialog_status = true;
        baqmt_detail_vm.is_play = true;
        $("#iframe_zfsyps").hide();
