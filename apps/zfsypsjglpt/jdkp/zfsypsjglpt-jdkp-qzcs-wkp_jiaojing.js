...
require('./zfsypsjglpt-jdkp-qzcs-wkp_jiaojing.css');
export const name = 'zfsypsjglpt-jdkp-qzcs-wkp_jiaojing';
...
let ajgl_session = (function () {
    let qzcs_xh = ''; //强制措施违法数据编号-主键
    return {
        getAjbh: function () {
            qzcs_xh = storage.getItem('zfsypsjglpt-jdkp-qzcs-current-qzcsData').xh;
            return qzcs_xh;
        }
    };
})();
...
avalon.component(name, {
    template: __inline('./zfsypsjglpt-jdkp-qzcs-wkp_jiaojing.html'),
    defaults: {
        ...
        ajgl_back(e) {
            ...
            $('.popover').hide();
            Tools.clearPlayer('');
            qzcs_ck_Man.jckpStatus = false;
            qzcs_ck_Man.fzkpStatus = false;
            ...
            //路由跳转至查询页面
            let init_data = storage.getItem("zfsypsjglpt-jdkp-qzcs-current-qzcsData");
            if (init_data) {
                if (init_data.sjqzcskp) {
                    avalon.history.setHash("/zfsypsjglpt-jdkp-sjqzcskp");
                } else {
            ...
            storage.removeItem("zfsypsjglpt-jdkp-qzcs-current-qzcsData");
        ...
        toggleMt(e, z, x) {
            ...
            var type = qzcs_ck_Man.glmt_mark;
            var aim = null;
            qzcs_ck_Man[type + '_data'].forEach(function (val, key) {
        ...
        playFile(e) {
            ...
            // qzcs_ck_Man.checkLooking = '正在查看：' + name;
            ...
            if (name && orgNameStr) {
                qzcs_ck_Man.checkLooking = '正在查看：' + orgNameStr + ' - ' + userNmaeAndCode + ' - ' + name;
            } else {
                qzcs_ck_Man.checkLooking = '正在查看：' + name;
            }
            var mark = qzcs_ck_Man.glmt_mark;
            ...
            ajax({
                ...
                if (ret.code == 0) {
                    ...
                    if (ret.data.saveTime == '已过期') {
                        ...
                        qzcs_ck_Man.imgff = false;
                        qzcs_ck_Man.ocxPlayer = false;
        ...
        qzcs_titleShow_kp: '考评', //考评|考评结果|核查处理|考评核查结果
        ...
        cjxx(kpType) {
            ...
            this.kpType = kpType;
            this.cjxx_show = -1;
            qzcs_ck_Man.cjxxInfo.selectEva = [];
            tempUnselected = [];
            if (kpType == 'jckp') {
        ...
        cjxxInfo: avalon.define({
            ...
            handleChangePassed(e) {
                let _this = this;
                _this.qzcsPassed = e.target.value == 'Y' ? true : false;
            ...
            handleChange(e) {
                ...
                if (tempArrChecked) {
                    _this.qzcsPassed = false; //如果选择了一项考评为 是，则不通过考评；
                }
            ...
            submitComment() {
                ...
                if (qzcs_ck_Man.cjxxInfo.commentValue.length > 500) {
                    notification.warn({
                        message: '评语最多输入500个字',
                        title: '温馨提示'
                    });
                ...
                if (qzcs_ck_Man.cjxxInfo.selectEva.length == 0) {
                    notification.warn({
                ...
                // if (qzcs_ck_Man.cjxxInfo.selectCheck[0].check == 'none') {
                //     notification.warn({
                //         message: '请选择考评是否通过!',
                //         title: '温馨提示'
                //     });
                ...
                // if (qzcs_ck_Man.cjxxInfo.commentValue != "" || (tempUnselected.length >= 1)) { //考评项 或 评语 二选一
                if (qzcs_ck_Man.cjxxInfo.commentValue != "") { //评语必填
        ...
        onInit(event) {
            ...
            qzcs_ck_Man.cjxx_show = '-2';
            //置空警情/媒体信息栏的初始值
            this.jbxxInfo.infomation = {};
            qzcs_ck_Man.cjxxInfo.selectCheck[0].check = 'none'; //初始化通过选项
            ...
            currentList.evaStatus = (storage.getItem("zfsypsjglpt-jdkp-qzcs-current-qzcsData").evaStatus) ? true : false;
            currentList.reviewStatus = (storage.getItem("zfsypsjglpt-jdkp-qzcs-current-qzcsData").reviewStatus) ? true : false;

            //请求警情信息并进行页面填充操作
            globalEvaList = {
                retData: []
            };
            qzcs_ck_Man.authority.SEARCH = true;

            Tools.searchCaseInfo(); //再次，解决ie和火狐执行顺序不一致bug，初始化基本信息和处警信息  


            qzcs_ck_Man.ocxPlayer = true;
                ...
                avalon.each(func_list, function (k, v) {
                    switch (v) {
                        ...
                        case "AUDIO_FUNCTION_JDKP_AJKP_JCKP":
                            qzcs_ck_Man.authority.JCKP = true;
                            break;
                        case "AUDIO_FUNCTION_JDKP_AJKP_FZKP":
                            qzcs_ck_Man.authority.FZKP = true;
                            break;
                        case "AUDIO_FUNCTION_JDKP_JTWFKP_QZCS_KP":
                            qzcs_ck_Man.authority.JTWFKP_KP = true;
                            break;
                    }
                });
                // 无考评权限时选中第一个tab
                if (!qzcs_ck_Man.authority.JTWFKP_KP) {
                    qzcs_ck_Man.jbxx();
                }
            });
        },
        onReady() {
            ...
            $(window).bind('beforeunload', function (event) { //ie
                ...
                if (global.location.hash.indexOf("zfsypsjglpt-jdkp-qzcs") > -1)
                    avalon.history.setHash("/zfsypsjglpt-jdkp-qzcs_jiaojing");
            });
...
let confirmCont = avalon.define({
    ...
    confirmOk() {
        ...
        if (confirmVm.operation_type == 1) {
            let clickType = qzcs_ck_Man.kpType;
            let tempType = qzcs_ck_Man.kpType == 'jckp' ? "BASIC" : "LEGAL";
            ajax({
                url: '/gmvcs/audio/force/eva',
                ...
                data: {
                    ...
                    evaInfo: {
                        evaItems: qzcs_ck_Man.cjxxInfo.selectEva,
                        comment: qzcs_ck_Man.cjxxInfo.commentValue,
                        passed: qzcs_ck_Man.cjxxInfo.qzcsPassed //是否通过
                    }
                },
                cache: false
            }).then(ret => {
...
function markRedIcon() {
    ...
    let temp = document.getElementById("commentBox");
    if (qzcs_ck_Man.cjxxInfo.commentValue == "") {
...
let Tools = {
    ...
    searchMediaByJqbh: function () {
        ...
        //     };
        //     qzcs_ck_Man.cj_data = [];
        //     qzcs_ck_Man.jq_nodata = !Boolean(qzcs_ck_Man.jq_data.length);
        //     qzcs_ck_Man.cj_nodata = !Boolean(qzcs_ck_Man.cj_data.length);
        ...
        ajax({
            ...
            if (ret.code == 0) {
                ...
                //处理选中已经总共数量的显示
                // qzcs_ck_Man.jq_data = ret.data.handelerBaseFiles;
                qzcs_ck_Man.cj_data = ret.data.handelerBaseFiles;
                qzcs_ck_Man.jq_nodata = !Boolean(qzcs_ck_Man.jq_data.length);
                qzcs_ck_Man.cj_nodata = !Boolean(qzcs_ck_Man.cj_data.length);
                this.configGlmtNumber(); //关联媒体总数和选中个数
    ...
    clearPlayer: function (type) {
        var playing = type == '_dia' ? tjgl_form : qzcs_ck_Man;
    ...
    searchCaseInfo: function () {
        // Tools.searchMediaByJqbh();      
        ajax({
            url: '/gmvcs/audio/force/search/' + ajgl_session.getAjbh(),
            // url: '/api/jdkpQZCS-hqxxxx',            
            method: 'get',
            data: '',
            cache: false
        }).then(ret => {
            ...
            if (ret.code == 0) {
                ...
                if (!ret.data) {
                    ...
                    qzcs_ck_Man.jbxxInfo.infomation = ret.data;
                    var type = qzcs_ck_Man.glmt_mark;
                    qzcs_ck_Man.glmt_total = qzcs_ck_Man[qzcs_ck_Man] ? qzcs_ck_Man[qzcs_ck_Man].length : 0;
                    //测试字段对接
                    var ret_data = ret.data;
                    ret_data.wfbh; //强制措施违法数据编号-主键
                    ...
                    ret_data.files; //强制措施违法数据关联到的媒体
                    qzcs_ck_Man.cj_data = ret_data.files; //简易程序违法数据关联到的媒体
                    if (qzcs_ck_Man.cj_data) {
                        qzcs_ck_Man.cj_data.forEach(function (val, key) {
                            val.checked = false;
                            val.importTime = formatDate(val.importTime);
                            val.startTime = formatDate(val.startTime);
                            val.duration = Tools.formatDuring(val.duration * 1000);
                        ...
                        qzcs_ck_Man.cj_nodata = true;
                    }
                    ret_data.evaInfo; //强制措施违法数据考评信息
                    ret_data.reviewInfo; //强制措施违法数据考评核查信息
                    ret.data.wfsj = formatDate(ret.data.wfsj);
                    qzcs_ck_Man.jbxxInfo.infomation = ret.data;

                    //处理案件信息
                    qzcs_ck_Man.checkLooking = (ret.data.wfxwmc || '-') + '(' + (ret.data.pzbh || '-') + ')';
                    ret.data.wfsj = formatDate(ret.data.wfsj);
        ...
        //kplb==2为超级管理员
        // if (kplb_type == 2) {
        //     qzcs_ck_Man.jckp_clickClass = "jbxx-btn";
        //     qzcs_ck_Man.mtxx_clickClass = 'cjxx-btn';
        //     qzcs_ck_Man.jbxx_clickClass = "cjxx-btn";
        //     qzcs_ck_Man.fzkp_clickClass = "cjxx-btn";
        //     qzcs_ck_Man.jckpStatus = true;
        //     qzcs_ck_Man.fzkpStatus = true;
        //     qzcs_ck_Man.kpType = 'jckp';
        //     qzcs_ck_Man.authority.JCKP = true;
        //     qzcs_ck_Man.authority.FZKP = true;
        //     if (currentList.jckp) {
        //         getEvaInfo("BASIC");
        ...
        qzcs_ck_Man.jbxx_clickClass = "cjxx-btn";
        qzcs_ck_Man.jckp_clickClass = "jbxx-btn";
        qzcs_ck_Man.jbxx_show = false;
        qzcs_ck_Man.cjxx_show = -1;
        qzcs_ck_Man.jckpStatus = true;
        qzcs_ck_Man.fzkpStatus = false;
        qzcs_ck_Man.kpType = 'jckp';
        ...
        if (currentList.evaStatus) {
            getEvaInfo("BASIC");
            qzcs_ck_Man.qzcs_titleShow_kp = '考评结果';
        } else {
            getPolicesituation();
            qzcs_ck_Man.qzcs_titleShow_kp = '考评';
        }


        // if ((qzcs_ck_Man.authority.JCKP == true)) {
        //     qzcs_ck_Man.fzkpStatus = false;
        //     switch (kplb_type) {
        //         case 0:
        //             if (currentList.jckp) {
        ...
        //             }
        //             //控制底部法制考评是否出现
        //             if ((policeType == true) && (qzcs_ck_Man.authority.FZKP == true) && (currentList.fzkp == true)) {
        //                 qzcs_ck_Man.fzkpStatus = true;
        //             }
        //             getSelectedItem("jckp");
        //             break;
        //         case 1:
        //             if (qzcs_ck_Man.authority.FZKP == true) {
        //                 qzcs_ck_Man.fzkpStatus = true;
        ...
        //                 getSelectedItem("jckp");
        //                 //法制考评有配置,且已考评过，则显示
        //                 if ((qzcs_ck_Man.authority.FZKP == true) && (currentList.fzkp == true)) {
        //                     qzcs_ck_Man.fzkpStatus = true;
        //                 }
        //             } else if ((qzcs_ck_Man.authority.FZKP == true) && (currentList.fzkp == true)) {
        //                 qzcs_ck_Man.fzkpStatus = true;
        //                 getEvaInfo("LEGAL");
        //                 getSelectedItem("fzkp");
        //             }
        //             break;
        //     }
        // } else if ((qzcs_ck_Man.authority.JCKP == false) && (qzcs_ck_Man.authority.FZKP == true)) {
        //     switch (kplb_type) {
        //         case 0:
    ...
    clickGlmt: function (type) {
        qzcs_ck_Man.glmt_name.forEach(function (val, key) {
            qzcs_ck_Man[val + '_clickClass'] = 'glmt_cj';
            qzcs_ck_Man['glmt_' + val + '_show'] = false;
        });
        qzcs_ck_Man[type + '_clickClass'] = 'glmt_jj';
        qzcs_ck_Man['glmt_' + type + '_show'] = true;
        qzcs_ck_Man.glmt_mark = type;
        Tools.configGlmtNumber();
    },
    configGlmtNumber: function () {
        var type = qzcs_ck_Man.glmt_mark;
        qzcs_ck_Man.glmt_total = qzcs_ck_Man[type + '_data'].length;
    },
    init: function () {
...
//已考评:获取强制措施考评项列表[基层考评跟法制考评复用:传类型进去]
function getEvaInfo(type) {
    ajax({
        url: '/gmvcs/audio/force/eva/info/' + ajgl_session.getAjbh(),
        ...
        if (ret.code == 0) {
            let tempEvaList = [];
            qzcs_ck_Man.noneKP_cont.alreadyEvaLists = [];
            if (!ret.data) {

            } else {
                if (ret.data.comment) {
                    qzcs_ck_Man.noneKP_cont.comment = ret.data.comment;
                } else {
                    qzcs_ck_Man.noneKP_cont.comment = "无";
                }
                ...
                //     qzcs_ck_Man.noneKP_cont.alreadyEvaLists = [];
                //     qzcs_ck_Man.noneKP_cont.noneEvaList = true;
                // } else {
                //     qzcs_ck_Man.noneKP_cont.alreadyEvaLists = tempEvaList;
                //     qzcs_ck_Man.noneKP_cont.noneEvaList = false;
                // }
                qzcs_ck_Man.noneKP_cont.result = (ret.data.passed ? '通过。原因：' : '不通过。原因：') + ret.data.result;
                qzcs_ck_Man.noneKP_cont.orgName = ret.data.orgName;
                qzcs_ck_Man.noneKP_cont.userName = ret.data.userName;
                qzcs_ck_Man.noneKP_cont.userCode = ret.data.userCode;
                qzcs_ck_Man.noneKP_cont.createTime = formatDate(ret.data.createTime);
...
function getSelectedItem(status) {
    switch (status) {
        ...
        case "fzkp":
            qzcs_ck_Man.jbxx_clickClass = "cjxx-btn";
            qzcs_ck_Man.fzkp_clickClass = 'jbxx-btn';
            qzcs_ck_Man.jbxx_show = false;
            qzcs_ck_Man.fzkpStatus = true;
            qzcs_ck_Man.kpType = 'fzkp';
            break;
    ...
    ajax({
        ...
        if (ret.code == 0) {
            if (ret.data.length != 0) {
                ...
                qzcs_ck_Man.cjxxInfo.selectEva = selectEva;
                qzcs_ck_Man.cjxxInfo.evaLists = ret.data;
            ...
            //展示未考评
            qzcs_ck_Man.cjxxInfo.commentValue = "";
            qzcs_ck_Man.cjxx_show = 0;
        }
    });
}
...
let filePlayer = (function () {
    ...
    return {
        play: function (type, rid, dia) {
            ...
            States.playing = dia == '_dia' ? tjgl_form : qzcs_ck_Man;
            ajax({
