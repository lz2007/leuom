 ...
 * @Date: 2018-09-07 10:42:38 
 * @Last Modified by: mikey.liangzhu
 * @Last Modified time: 2019-08-07 09:28:26
 */
...
export const name = "zfsypsjglpt-jdkp-kphczl_jiaojing";
...
avalon.component(name, {
    template: __inline("./zfsypsjglpt-jdkp-kphczl_jiaojing.html"),
    defaults: {
        showScroll: false,
        isPolice: true, // 是否为警员        
        authority: { // 按钮权限标识
        ...
        // 查询按钮
        searchBtn() {
            // Gm_tool.resetTableWidth(this.tableConfig);                
            this.search_list();
        },
        ...
        get_table_list() {
            ...
            ajax({
                ...
                if (result.code == 0) {
                    // let ret_data = [];
                    let ret_data = result.data.currentElements;
                    ...
                    //     ret_data[index].jtwfcctgs = item.jtwfcctgs; 
                    //     ret_data[index].jtwfcctgbl = Gm_reg.positive.test(Number(item.jtwfcctgbl)) ?　((cl.toFixed(2)) + '%') : item.jtwfcctgbl; 
                    //     ret_data[index].jtwfccbtgs = item.jtwfccbtgs; 
                    //     ret_data[index].jtwfccbtgbl = Gm_reg.positive.test(Number(item.jtwfccbtgbl)) ?　((dl.toFixed(2)) + '%') : item.jtwfccbtgbl;
                    //     ret_data[index].jtwffhqkss = item.jtwffhqkss;                         
                    //     ret_data[index].jtwffhqkbss = item.jtwffhqkbss; 
                    //     ret_data[index].jtwffhkfqk = item.jtwffhkfqk; 
                    //     ret_data[index].sgclcctgs = item.sgclcctgs; 
                    //     ret_data[index].sgclcctgbl =　Gm_reg.positive.test(Number(item.sgclcctgbl)) ?　((el.toFixed(2)) + '%') : item.sgclcctgbl;
                    //     ret_data[index].sgclccbtgs = item.sgclccbtgs;                     
                    //     ret_data[index].sgclccbtgbl = Gm_reg.positive.test(Number(item.sgclccbtgbl)) ?　((fl.toFixed(2)) + '%') : item.sgclccbtgbl; 
                    //     ret_data[index].sgclfhqkss = item.sgclfhqkss; 
                    //     ret_data[index].sgclfhqkbss = item.sgclfhqkbss; 
                    //     ret_data[index].sgclfhkfqk = item.sgclfhkfqk;     
                    // });
                    ...
                    storage.setItem('zfsypsjglpt-jdkp-kpcczl', storageData, 0.5);
        ...
        config() {
            let hash = "/zfsypsjglpt-jdkp-kphczl_jiaojing";
            avalon.history.setHash(hash);
        },
        tableConfig: {
            body: 'zcqktj-list-content',
            name: 'kpcczl_jj_table_width',
        ...
        onInit(e) {
            ...
            ajax({
                ...
                let getFromStorage = !!storage.getItem('zfsypsjglpt-jdkp-kpcczl'); // getFromStorage 判断是否需要初始化页面；false为重新从后台拿数据初始化，true为从Storage拿数据填充
                let storageData = storage.getItem('zfsypsjglpt-jdkp-kpcczl');
                ...
                if (!getFromStorage) {
                    ...
                    if (!!window.hadSaveConf) { // hadSaveConf 是否保存权重配置数据，true为重新更新数据，false时直接取localstorage的数据
                        // 修改权重配置后重置查询条件等数据
                        storage.removeItem('zfsypsjglpt-jdkp-kpcczl');
            ...
            menuServer.menu.then(menu => {
                ...
                avalon.each(list, function (index, el) {
                    if (/^AUDIO_FUNCTION_JDKP_KPCCZL/.test(el))
                ...
                avalon.each(func_list, function (k, v) {
                    switch (v) {
                        case "AUDIO_FUNCTION_JDKP_KPCCZL_SEARCH":
                ...
                if (false == _this.authority.SEARCH && false == _this.authority.QZPZ)
                    $(".zhkp_tabCont").css("top", "0px");
...
let zhkp_table = avalon.define({
    $id: "zfsypsjglpt_jdkp_kpcczl_table",
    isHeaderTitle: false,
    table_loading: false,
    zhkp_table_data: [],
});
...
var tab = ['zfsypsjglpt-jdkp-jycxkp', 'zfsypsjglpt-jdkp-fxccf', 'zfsypsjglpt-jdkp-qzcs'];
var tab_data = {
    'zfsypsjglpt-jdkp-jycxkp': {
        orgPath: '',
        wfsjEnd: '',
        wfsjStart: '',
        userCode: '',
        ...
        reviewStatus: 'ALL',
        reviewResult: 'ALL',
        timeStatus: 'last-week',
        page: 0,
        pageSize: 20,
    },
    'zfsypsjglpt-jdkp-fxccf': {
        wfsjEnd: '',
        wfsjStart: '',
        userCode: '',
        hphm: '',
...
function changeStoraeProp(prop, value) {
    tab.forEach((val, index) => {
        ...
        storagedata.page = 0;
        storagedata.wfsjStart = data_mid.zhkp_startTime;
        storagedata.wfsjEnd = data_mid.zhkp_endTime;
...
function changeStoragePropForSgcl(prop, value) {
    let storagedata = storage.getItem('zfsypsjglpt-jdkp-sgclkp');
    let copyObj = JSON.parse(JSON.stringify(sgclkpdata));
    storagedata = storagedata ? JSON.parse(storagedata) : copyObj;
    storagedata.evaStatus = copyObj.evaStatus;
    storagedata.evaResult = copyObj.evaResult;
    storagedata.reviewStatus = copyObj.reviewStatus;
    ...
    storagedata.orgCode = data_mid.orgCode;
    storagedata.page = 0;
    storagedata.sgsjStart = data_mid.zhkp_startTime;
    storagedata.sgsjEnd = data_mid.zhkp_endTime;
    storage.setItem('zfsypsjglpt-jdkp-sgclkp', JSON.stringify(storagedata), 0.5);
}
let data_mid = {};
var zhkp_tableObject = $.tableIndex_zhkp({ //初始化表格jq插件
    id: 'kpcczl_table_id',
    controller: 'kpcczl_table_vm',
    currentPage: 1,
    prePageSize: 20,
    actions: function (type, text, record, index, mark) {
        // console.log(arguments)
        // console.log(type, text, record, index, mark)
        // if (text == 0) {
        ...
        window.if_kpcczl_hc = true;
        window.igdd = true;
        window.checkChildren = data_mid.orgId == zhkp_vm.zhkp_tree.tree_key ? false : true;

        // 事故处理
        if (type == 'sgcl') {
            switch (mark) {
                case 'sg_passed':
                    changeStoragePropForSgcl('evaResult', '1');
                    break;
                case 'sg_no_passed':
                    changeStoragePropForSgcl('evaResult', '0');
                    break;
                case 'real':
                    changeStoragePropForSgcl('reviewResult', '1');
                    break;
                case 'no_real':
                    changeStoragePropForSgcl('reviewResult', '0');
                    break;
