...
require('./zfsypsjglpt-jdkp-kpcczl-ck-fxccf.css');
export const name = 'zfsypsjglpt-jdkp-kpcczl-ck-fxccf';
...
function Tools() {
    ...
    this.getTree = function () {
        ajax({
            ...
            var jqgl_form_data = Gm_tool.getStorage('zfsypsjglpt-jdkp-kpcczl-ck-fxccf');
            ...
            ajax({
                ...
                if (ret.code == 0) {
                    ...
                    if (ret.data) {
                        var $tree_target = $.fn.zTree.getZTreeObj($('#fxccf_tree .ztree').attr('id'));
...
avalon.component(name, {
    template: __inline('./zfsypsjglpt-jdkp-kpcczl-ck-fxccf.html'),
    defaults: {
        ...
        searchForm_bjsj: avalon.define({
            ...
            callback: {
                ...
                others(ret) {
                    ...
                    $('.fxccf-form').find($("[placeholder = '开始时间']")).val(Gm_format.formatDate(+ret.end, true));
                    $('.fxccf-form').find($("[placeholder = '结束时间']")).val(Gm_format.formatDate(+ret.now, true));
                }
            },
            calcutor: null,
        ...
        //input控件和叉叉按钮
        fxccf_close_userCode: false,
        fxccf_close_wfbh: false,
        fxccf_close_hphm: false,
        fxccf_close_jdsbh: false,
        fxccf_close_wfdz: false,
        close_click(e) {
        ...
        input_focus(e) {
            this['fxccf_close_' + e] = true;
        },
        input_blur(e) {
            this['fxccf_close_' + e] = false;
        },

        //关联媒体
        fxccf_glmt: avalon.define({
            $id: 'fxccf_glmt',
            glmt: ['0'],
            fxccf_glmt_Change(e, a) {
        ...
        actions(type, text, record, index) {
            ...
            if (type == 'checkLook') {
                window.kpcczl_fxccf = record.wfbh;
        ...
        ajax_table(initMark) {
            ...
            this.loading = true;
            seachParams.timeStatus = this.searchForm_bjsj.calcutor.getStatus();
        ...
        getTableData(seachParams, page) {
            ajax({
                ...
                if (ret.code == 0) {
                    Gm_tool.setStorage('zfsypsjglpt-jdkp-kpcczl-ck-fxccf', jqglMan.$searchForm.record, 0.5, function () {
        ...
        onInit(event) {
            ...
            if (jqgl_form_data) {
                ...
                ({
                    ...
                    jdsbh: this.jdsbh,
                    wfdz: this.wfdz,
                    glmt: this.fxccf_glmt.glmt[0],
                    evaStatus: this.fxccf_kpzt.kpzt[0],
                    evaResult: this.fxccf_kpjg.kpjg[0],
                    reviewStatus: this.fxccf_hczt.hczt[0],
                    reviewResult: this.fxccf_hcjg.hcjg[0],
                } = jqgl_form_data);
                ...
                avalon.each(func_list, function (k, v) {
                    switch (v) {
                        case "AUDIO_FUNCTION_ZFDA_JQGL_CHECK":
                            jqglMan.fxccf_check.authority.CHECK = true;
                            break;
                        case "AUDIO_FUNCTION_ZFDA_JQGL_SEARCH":
                            jqglMan.fxccf_check.authority.SEARCH = true;
                            break;
                    }
                });

                if (false == jqglMan.fxccf_check.authority.CHECK)
                    jqglMan.fxccf_check.authority.OPT_SHOW = true;

                // 防止查询无权限时页面留白
                if (false == jqglMan.fxccf_check.authority.SEARCH)
                    $('#aqglTable').css('top', '6px');
            });
        },
        onReady() {
...
let tableBodyJQGL = avalon.define({
