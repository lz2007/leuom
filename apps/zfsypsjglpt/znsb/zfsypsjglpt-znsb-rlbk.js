...
export const name = "zfsypsjglpt-znsb-rlbk";
require("./zfsypsjglpt-znsb-rlbk.less");
...
avalon.component(name, {
    template: __inline("./zfsypsjglpt-znsb-rlbk.html"),
    defaults: {
        ...
        actions(type, text, record, index) {
            if (type == "check") {
                storage.setItem('zfsypsjglpt-znsb-rlbk-recordID', record);
                $('.zfsypsjglpt-map').css({
                    'position': 'absolute',
                    'width': 'auto',
                    'left': '210px',
                ...
                avalon.history.setHash("/zfsypsjglpt-znsb-rlbk-detail");
            }
            if (type == "comparison") {
                rlbk_dialog_vm.scene_img = record.shootPersonImgFilePath;
                rlbk_dialog_vm.recognition_img = record.personRegImgFilePath;
                rlbk_dialog_vm.title = record.personName ? record.personName : '-' + '的' + getLan().imageContrast;
        ...
        onInit(e) {
            ...
            let storage_item = storage.getItem('zfsypsjglpt-znsb-rlbk');
            ...
            ajax({
                ...
                if(result.data.length === 0) {
                    ...
                    rlbk_vm.noOrgData = true;
                    return;
                }
                rlbk_vm.noOrgData = false;
                ...
                avalon.each(func_list, function (k, v) {
                    switch (v) {
                        case "AUDIO_FUNCTION_ZNSB_RLSB_CK":
                            ...
                            break;
                        case "AUDIO_FUNCTION_ZNSB_RLSB_CX":
        ...
        opt_rlbk: avalon.define({
            $id: "opt_rlbk",
            authority: { // 按钮权限标识
                "CHECK": false, //智能识别-人脸识别-查看
                "SEARCH": false, //智能识别-人脸识别-查询
        ...
        getTableList() {
            ...
            ajax({
                ...
                if (!result.data.overLimit && result.data.totalElements == 0) {
                    ...
                    storage.setItem('zfsypsjglpt-znsb-rlbk', storage_item, 0.5);
                    return;
                }
