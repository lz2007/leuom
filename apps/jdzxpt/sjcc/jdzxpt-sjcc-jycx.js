 ...
 * @LastEditTime: 2019-01-09 17:57:06
...
export const name = "jdzxpt-sjcc-jycx";
import "./jdzxpt-sjcc-jycx.less";
...
avalon.component(name, {
    template: __inline("./jdzxpt-sjcc-jycx.html"),
    defaults: {
        ...
        onInit(e) {
            ...
            let storage_item = storage.getItem('jdzxpt-sjcc-jycx');
            ...
            //         jycx_time_range.time_range[0] = storage_item.range_flag.toString();
            //     }
            //     jycx_startTime_vm.jycx_startTime = storage_item.ajax_data.startTime;
            //     jycx_endTime_vm.jycx_endTime = storage_item.ajax_data.endTime;

            //     this.included_status = storage_item.ajax_data.includeChild;
            //     this.generateNum = storage_item.ajax_data.generateNum || ""; //生成条数
            //     this.generateNum_title = this.generateNum || jycx_vm.jycx_txt.supportPositiveIntegersAnd0;
        ...
        onReady() {
            ...
            menuServer.menu.then(menu => {
                ...
                avalon.each(list, function (index, el) {
                    if (/^EVA_FUNCTION_JDKP_SJCC_JYCX/.test(el)) {
                        avalon.Array.ensure(func_list, el);
                    }
                });
                avalon.each(func_list, function (k, v) {
                    switch (v) {
                        case "EVA_FUNCTION_JDKP_SJCC_JYCX_SEARCH":
        ...
        ajaxDepTree(storage_item) {
            ...
            ajax({
                ...
                jycx_tree_vm.tree_data = deptemp;
                if (storage_item) {
                    jycx_tree_vm.tree_code = storage_item.ajax_data.orgPath;
                    jycx_tree_vm.tree_key = storage_item.tree_key;
                    jycx_tree_vm.tree_title = storage_item.tree_title;
                } else {
                    jycx_tree_vm.tree_code = deptemp[0].path;
                    jycx_tree_vm.tree_key = deptemp[0].key;
                    jycx_tree_vm.tree_title = deptemp[0].title;
                }
        ...
        ajaxIsCreated() {
            ajax({
                url: '/gmvcs/audio/violation/random/result/empty',
                method: 'get',
                data: {}
            }).then(result => {
                jycx_vm.isCreated = (result.data == true ? 1 : 2);
            });
        },
        //部门树下拉框是否包含子部门
        clickBranchBack(e) {
        ...
        //获取表格数据
        getTableList() {
            this.startTime = jycx_startTime_vm.jycx_startTime;
            this.endTime = jycx_endTime_vm.jycx_endTime;
            let ajax_data = {
                "endTime": getTimeByDateStr(this.endTime, true),
                "includeChild": this.included_status,
                "orgPath": jycx_tree_vm.curTree || jycx_tree_vm.tree_code,
                "page": 0,
            ...
            ajax_data.orgName = jycx_tree_vm.tree_title;
            ajax_data.generateNum = -1; //只有当可生成并且输入框有值的时候，才不传-1值
            if (jycx_vm.isCreated == 1 && jycx_vm.generateNum) { //可生成的时候且输入框有值的时候
                ajax_data.generateNum = $.trim(jycx_vm.generateNum);
            }
            if (jycx_vm.generateNum > 100) {
        ...
        ajaxTableData(ajax_data) {
            ajax({
                url: '/gmvcs/audio/violation/random/generate',
                method: 'post',
                data: ajax_data
            }).then(result => {
                if (result.code != 0) {
                ...
                if (!result.data.pageObject.overLimit && result.data.pageObject.totalElements == 0) {
                    ...
                    let storage_item = {
                        "ajax_data": temp_data,
                        "total": 0,
                        "tree_key": jycx_tree_vm.tree_key,
                        "tree_title": jycx_tree_vm.tree_title
                    };
                    storage.setItem('jdzxpt-sjcc-jycx', storage_item, 0.5);
                    return;
                }
                if (!this.change_page) {
                ...
                // if (dep_switch && ret_data.length!= 0) {//黔西南 部门提示需求功能 开关
                //     let times = 0;
                //     var getFullName = new Promise(function(resolve, reject){
                //             for(let currIndex = 0; currIndex <  ret_data.length; currIndex++) {
                //                 let val =  ret_data[currIndex];
                //                 ajax({
                //                     url: `/gmvcs/uap/org/getFullName?orgCode=${val.orgCode}&prefixLevel=${prefixLevel}&separator=${separator}`,
                //                     method: 'get'
                //                 }).then(res => {
                //                     val.orgCode = res.data;
                //                     times = times + 1;
                //                 }).then(function(){
                //                     if(times >= ret_data.length-1) {
                //                         resolve('end');
                //                     }
                //                 });
                //             };
                ...
                //     getFullName.then(x => {
                //         setTimeout(function(){
                //             o_this.table_list = ret_data;
                //             o_this.loading = false;
                //             //设置 头部显示的查询参数： 部门 抽查条数 时间范围
                //             jycx_vm.depTreeTitle = result.data.criteria.orgName;
                //             jycx_vm.generateNum = result.data.criteria.generateNum;
                //             jycx_vm.startTime = moment(result.data.criteria.startTime).format("YYYY-MM-DD");
                //             jycx_vm.endTime = moment(result.data.criteria.endTime).format("YYYY-MM-DD");
                //             let storage_item = {
                //                 "ajax_data": temp_data,
                //                 "range_flag": jycx_time_range.range_flag,
                //                 "total": o_this.table_pagination.total,
                //                 "tree_key": jycx_tree_vm.tree_key,
                //                 "tree_title": jycx_tree_vm.tree_title
                //             };
                //             storage.setItem('jdzxpt-sjcc-jycx', storage_item, 0.5);
            ...
            switch (val) {
                case 'generateNum':
                    if (e.target.value != "") {
                        ...
                        this.generateNum_title = jycx_vm.jycx_txt.supportPositiveIntegersAnd0;
                    }
                    break;
            }
            if (e.keyCode == "13") {
        ...
        input_focus(e) {
            switch (e) {
                case 'generateNum':
                    ...
                    $(".znsb_jycx .dataFormBox .generateNum").width($(".jycx_input_panel").innerWidth() - 33);
                    break;
            }
        },
        input_blur(e) {
            switch (e) {
                case 'generateNum':
                    ...
                    $(".znsb_jycx .dataFormBox .generateNum").width($(".jycx_input_panel").innerWidth() - 23);
                    break;
            }
        },
        //重置时间组件,默认时间是本周
        resetDatepicker() {
            jycx_time_range.time_range[0] = "0";
            jycx_time_range.range_flag = 0;
...
//部门树
let jycx_tree_vm = avalon.define({
    $id: "jycx_tree",
    tree_data: [],
    tree_key: "",
    tree_title: "",
...
let jycx_endTime_vm = avalon.define({
    ...
    handlerChange(e) {
        this.jycx_endTime = e.target.value;
    },
});
//获取语言对象
function getLan() {
