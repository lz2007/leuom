 ...
 * @Date: 2018-10-23 11:13:59 
 * @Last Modified by: mikey.zhaopeng
 * @Last Modified time: 2019-11-19 17:49:54
 */
/*
 * @Author: mikey.liangzhu 
 * @Date: 2018-10-23 11:13:58 
 * @Last Modified by:   mikey.liangzhu 
 * @Last Modified time: 2018-10-23 11:13:58 
 */
/*
 * @Author: mikey.liangzhu 
 * @Date: 2018-10-23 11:13:53 
 * @Last Modified by:   mikey.liangzhu 
 * @Last Modified time: 2018-10-23 11:13:53 
 */
/**
...
export const name = 'tyywglpt-sbzygl-zdsbgl-main';
...
require('./tyywglpt-sbzygl-zdsbgl-main.css');
let vm = null,
    sbzygl = null,
    enableQuery = true,
    queryTimer = null,
...
avalon.component(name, {
    template: __inline('./tyywglpt-sbzygl-zdsbgl-main.html'),
    defaults: {
        ...
        onInit(event) {
                ...
                avalon.each(func_list, function (k, v) {
                    switch (v) {
                        ...
                        case "UOM_FUNCTION_SBZYGL_ZFJLYGL_UPGRADE":
                            ...
                            break;
                        case "UOM_FUNCTION_SBZYGL_ZFJLYGL_SBPZ":
                            _this.authority.CONFIG = true;
                            break;
                        case "UOM_FUNCTION_SBZYGL_ZFJLYGL_UPGRADE_WS_DIRECT":
            ...
            this.$watch('dataJson', (v) => {
                if (v) {
                    ...
                    this.sn = v.sn || "";
                    this.included_status = v.included_status || false;
                }
            });
        },
        onReady() {
            this.dataStr = storage.getItem(name);
            // this.dataJson = this.dataStr ? JSON.parse(this.dataStr) : null;//tyywglpt-sbzygl-zdsbgl-main无需tyywglpt-sbzygl-zdsbgl跳转进入时有缓存需求。
            this.dataJson = null;//由tyywglpt-sbzygl-zdsbgl进入tyywglpt-sbzygl-zdsbgl-main后tyywglpt-sbzygl-zdsbgl-main无缓存需求。

            this.recordStr = storage.getItem('zdsbgl_vm_currentRecord');
            this.recordJson = this.recordStr ? JSON.parse(this.recordStr) : null;
            if(!this.recordJson){//刷新后注册时间默认为不限
                this.timeMode = 4;
            }
            //表头宽度设置
            ...
            this.fetchOrgData(() => {
                let timer = setInterval(() => {
                    ...
                    if (vm.typeOk && vm.statusOk && vm.manufacturerOk && vm.modelOk && length >= 7) {
                        if(_this.recordStr){//如果是跳转进来则自动查询数据
                            vm.fetchList();
                        }else{//如果是本页刷新则不主动查询数据                            
                            if(isTableSearch){// 表格是否默认查询，默认false true--查询  false--不查询
                                vm.fetchList();
                            }else{
                                vm.isNull = true;
                            }
            ...
            uploader = new plupload.Uploader({
                ...
                url: "http://" + window.location.host + apiUrl + '/gmvcs/uom/device/gb28181/v1/device/register/excel', //服务器端的上传页面地址
        ...
        returnBtn() {
            storage.removeItem('zdsbgl_vm_currentRecord');
            avalon.history.setHash('/tyywglpt-sbzygl-zdsbgl');
        },
        onDispose() {
        ...
        getDefaultStatus(statusOptions, dataJson) {
            ...
            return statusOptions.length > 0 ? (dataJson ? dataJson.status : statusOptions[statusOptionsNum].value) : '';            
        ...
        handleTreeChange(e, selectedKeys) {
            if (this.title_index != null) {
                let html_orgname = $('.tyywglpt-search-box #zfygl-tree-select-1 .ztree li ul li a');
        ...
        extraExpandHandle(treeId, treeNode, selectedKey) {
            let data_zfycgslgl = storage.getItem('tyywglpt-sbzygl-zdsbgl-main-cgsl');
        ...
        handleRegistermodel() {
            let adress = encodeURIComponent("/static/batchRegistryModel.xlsx")
        ...
        handleConfig() {
            ...
            dialogConfigVm.fetchManuToAlarmTime();// 获取告警时间
            dialogConfigVm.fetchManuToTransResolution(); //获取网传分辨率
            dialogConfigVm.fetchManuToRecordResolution();// 获取本地录像分辨率
        ...
        handleCheckAll(e) {
            sbzygl.handleCheckAll(e, (list) => {
                ...
                if (checkedDataLength > 0 && this.checkedData[0].online >0) {
        ...
        handleCheck(index, record, e) {
            sbzygl.handleCheck(index, record, e, (hasChecked, record) => {
                ...
                if (checkedDataLength && this.checkedData[0].online >0) {
                    this.selectedIsOnline = true;
                } else {
                    this.selectedIsOnline = false;
                }
        ...
        query() {
            if (enableQuery) {
                this.current = 1;
                this.fetchList();
                let data_zfycgslgl_json = storage.getItem('tyywglpt-sbzygl-zdsbgl-main-cgsl') ? JSON.parse(storage.getItem('tyywglpt-sbzygl-zdsbgl-main-cgsl')) : null;
                ...
                if (data_zfycgslgl_json != null && data_zfygl_json != null) {
                    if (data_zfycgslgl_json.orgName) {
                        if (data_zfycgslgl_json.orgName != data_zfygl_json.orgName) {
                            ...
                            storage.setItem('tyywglpt-sbzygl-zdsbgl-main-cgsl', JSON.stringify(data_str3), 0.5);
                        }
                    }
                }
                enableQuery = false;
        ...
        fetchList() {
            ...
            let newData = {
                ...
                "isStandbyMachine": 2 // 2 为 执法仪， 1 为 备机 ， 0 为不限
            }
...
const dialogConfigVm = avalon.define({
    ...
    show: false,
    defaultAlarmTime:'',
    defaultTransResolution:'',
    defaultRecordResolution:'',
    alarmTimeOptions:[],//设备静止告警时间
    transResolutions:[],//网传分辨率
    recordResolutions:[],//本地录像分辨率
    inputJson: {
        "acutance":"100",
        "bitRate":"600000",
        "brightness":"100",
    ...
    validJson:{
        ...
        "volume":true,
        "alarmTime":true,
        "defaultTransResolution":true,
        "defaultRecordResolution":true,
    },
    handleDeviceConfigChange(e){
        this.validJson.alarmTime = true;
        this.defaultAlarmTime = e.target.value;
    },
    handleTransResolutionChange(e){//网传分辨率
        this.validJson.alarmTime = true;
        this.defaultTransResolution = e.target.value;
    },
    handleRecordResolutionChange(e){//本地录像分辨率
        this.validJson.alarmTime = true;
        this.defaultRecordResolution = e.target.value;
    },
    handleOk() {
        // 转换时间戳
        ...
        let data = {
            'acutance':  this.inputJson.acutance,
            'bitRate':  this.inputJson.bitRate,
            'brightness':  this.inputJson.brightness,
            'contrast':  this.inputJson.contrast,
            'deviceId':  deviceId,
            'deviceName':  deviceName,
            'deviceStateRate':  this.inputJson.deviceStateRate,
            'frameRate':  this.inputJson.frameRate,
            'locationRate':  this.inputJson.locationRate,
            'positionStopRate':  Number(this.defaultAlarmTime),
            'recordFrameRate':  this.inputJson.recordFrameRate,
            'recordResolution':  this.defaultRecordResolution,
            'transResolution':  this.defaultTransResolution,
            'volume':  this.inputJson.volume,
        };
    ...
    // 获取网传分辨率
    fetchManuToTransResolution() {    
        this.transResolutions= [
            {
                value:'720',
                label:"720P"
            },
        ...
        this.defaultTransResolution = this.transResolutions.length > 0 ? this.transResolutions[0].value : "";//默认选中的告警时间
    },
    // 获取本地录像分辨率
    fetchManuToRecordResolution() {    
        this.recordResolutions= [
            {
                value:'720',
                label:"720P"
            },
        ...
        this.defaultRecordResolution = this.recordResolutions.length > 0 ? this.recordResolutions[0].value : "";//默认选中的告警时间
    },
    handleCancel() {
        this.initDialog();
    },
...
function registerOrModify(url, dialogVm, successMsg, comfirmDump) {
    ...
    avalon.type(data) == 'array' ? (data[0].isStandbyMachine =  2) : (data.isStandbyMachine = 2);
