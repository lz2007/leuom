...
require('./zfsypsjglpt-jdkp-ajkp-wkp_gongan.css');
export const name = 'zfsypsjglpt-jdkp-ajkp-wkp_gongan';
...
let ajgl_session = (function () {
    ...
    return {
        getAjbh: function () {
            ajbh = storage.getItem('zfsypsjglpt-jdkp-ajkpQjzb-current-ajkpData').ajbh_ajkp;
            return ajbh;
        }
    };
})();
...
avalon.component(name, {
    template: __inline('./zfsypsjglpt-jdkp-ajkp-wkp_gongan.html'),
    defaults: {
        ...
        ajgl_back(e) {
            ...
            storage.removeItem("zfsypsjglpt-jdkp-ajkpQjzb-current-ajkpData");
            $("#gm_webplayer").hide();
            if (this.ocxPlayer) {
                $("#gm_webplayer").hide();
        ...
        playFile(e) {
            ...
            if (name && orgNameStr) {
                ajgl_ck_Man.checkLooking = '正在查看：' + orgNameStr + ' - ' + userNmaeAndCode + ' - ' + name;
            } else {
                ajgl_ck_Man.checkLooking = '正在查看：' + name;
            }
        ...
        cjxxInfo: avalon.define({
            ...
            handleChangePassed(e) {
                let _this = this;
                _this.ajkpPassed = e.target.value == 'Y' ? true : false;
            ...
            handleChange(e) {
                ...
                if (tempArrChecked) {
                    _this.ajkpPassed = false; //如果选择了一项考评为 是，则不通过考评；
                }
            ...
            submitComment() {
                ...
                // if (ajgl_ck_Man.cjxxInfo.selectCheck[0].check == 'none') {
                //     notification.warn({
                //         message: '请选择考评是否通过!',
                //         title: '温馨提示'
                //     });
                ...
                // if (ajgl_ck_Man.cjxxInfo.commentValue != "" || (tempUnselected.length >= 1)) { //考评项或 评语二选一
                if (ajgl_ck_Man.cjxxInfo.commentValue != "") { //评语为必填项
        ...
        onInit(event) {
            ...
            currentList.jckp = (storage.getItem("zfsypsjglpt-jdkp-ajkpQjzb-current-ajkpData").jckpStatus) ? true : false;
            currentList.fzkp = (storage.getItem("zfsypsjglpt-jdkp-ajkpQjzb-current-ajkpData").fzkpStatus) ? true : false;
                ...
                avalon.each(func_list, function (k, v) {
                    switch (v) {
                        ...
                        case "AUDIO_FUNCTION_JDKP_AJKP_FZKP":
                            ...
                            break;
                        case "AUDIO_FUNCTION_JDKP_AJKP_KP":
                            ajgl_ck_Man.authority.KP = true;
                            break;
                    }
                });
        ...
        onReady() {
            ...
            $(window).bind('beforeunload', function (event) { //ie
                ...
                if (global.location.hash.indexOf("zfsypsjglpt-jdkp-ajkp") > -1)
                    avalon.history.setHash("/zfsypsjglpt-jdkp-ajkp");
            });
...
let confirmCont = avalon.define({
    ...
    confirmOk() {
        ...
        if (confirmVm.operation_type == 1) {
            let clickType = ajgl_ck_Man.kpType;
            let tempType = ajgl_ck_Man.kpType == 'jckp' ? "BASIC" : "LEGAL";
            ajax({
                url: '/gmvcs/audio/case/eva',
                method: 'post',
                data: {
                    // "type": tempType,
                    // "dataType": "CM",
                    ...
                    evaInfo: {
                        evaItems: ajgl_ck_Man.cjxxInfo.selectEva,
                        comment: ajgl_ck_Man.cjxxInfo.commentValue,
                        passed: ajgl_ck_Man.cjxxInfo.ajkpPassed //是否通过
                    }
                },
                cache: false
            }).then(ret => {
...
function markRedIcon() {
    ...
    let temp = document.getElementById("commentBox");
    if (ajgl_ck_Man.cjxxInfo.commentValue == "") {
...
let Tools = {
    ...
    searchMediaByJqbh: function () {
        ...
        ajax({
            ...
            if (ret.code == 0) {
                ...
                let cjrxmTemp = [];
                for (let i = 0; i < ret.data.cjrxm.length; i++) {
                    cjrxmTemp.push(ret.data.cjrxm + '(' + ret.data.cjr + ')');
                }
                ret.data.zbmjxm = zbmjxmTemp;
                ret.data.cjrxm = cjrxmTemp;
        ...
        ajax({
            ...
            if (storage && storage.getItem) {
                ...
                if (storage.getItem('zfsypsjglpt-jdkp-ajkp')) {
                    ajgl_form_data = JSON.parse(storage.getItem('zfsypsjglpt-jdkp-ajkp'));
                }
            } else {
    ...
    searchCaseInfo: function () {
        ...
        // 无考评权限时选中第一个tab
        if (!ajgl_ck_Man.authority.KP) {
            ajgl_ck_Man.jbxx();
        } else {
            ajgl_ck_Man.cjxx('jckp');
        }
...
//已考评:获取案件考评项列表[基层考评跟法制考评复用:传类型进去]
function getEvaInfo(type) {
    ajax({
        url: '/gmvcs/audio/case/eva/info/' + ajgl_session.getAjbh(),
        method: 'get',
        data: {
            // type: type,
            // dataType: "CM",
        ...
        if (ret.code == 0) {
            let tempEvaList = [];
            ajgl_ck_Man.noneKP_cont.alreadyEvaLists = [];
            if (!ret.data) {

            } else {
                if (ret.data.comment) {
                    ajgl_ck_Man.noneKP_cont.comment = ret.data.comment;
                } else {
                    ajgl_ck_Man.noneKP_cont.comment = "无";
                }
                ajgl_ck_Man.noneKP_cont.result = (ret.data.passed ? '通过。原因：' : '不通过。原因：') + ret.data.result;
                ajgl_ck_Man.noneKP_cont.orgName = ret.data.orgName;
                ajgl_ck_Man.noneKP_cont.userName = ret.data.userName;
                ajgl_ck_Man.noneKP_cont.userCode = ret.data.userCode;
                ajgl_ck_Man.noneKP_cont.createTime = formatDate(ret.data.createTime);
...
function getSelectedItem(status) {
    switch (status) {
        case "jckp":
            ...
            ajgl_ck_Man.jbxx_clickClass = "cjxx-btn";
            ajgl_ck_Man.jckpStatus = true;
            ajgl_ck_Man.jbxx_show = false;
            ajgl_ck_Man.kpType = 'jckp';
            break;
        case "fzkp":
            ajgl_ck_Man.jbxx_clickClass = "cjxx-btn";
            ajgl_ck_Man.fzkp_clickClass = 'jbxx-btn';
            ajgl_ck_Man.jbxx_show = false;
            ajgl_ck_Man.fzkpStatus = true;
            ajgl_ck_Man.kpType = 'fzkp';
            break;
...
function getPolicesituation() {
    //获取警情考评项列表
    ajax({
        url: '/gmvcs/audio/eva/items/casemain',
        //		url: '/api/casemain',
        method: 'get',
        data: {},
        cache: false
        ...
        if (ret.code == 0) {
            if (ret.data.length != 0) {
                ...
                ajgl_ck_Man.cjxxInfo.selectEva = selectEva;
                ajgl_ck_Man.cjxxInfo.evaLists = ret.data;
            ...
            //展示未考评
            ajgl_ck_Man.cjxxInfo.commentValue = "";
            ajgl_ck_Man.cjxx_show = 0;
        }
    });
}
