...
let tableBodyAJGL = avalon.define({ //表格定义组件
    ...
    dragStorageName: 'zfda-jtwf-jycx-tableDrag-style', //表格拖动样式style存储storage名称，另外：在表格内所有元素中切记不要使用style定义样式以免造成影响
    handle: function (type, record) { //操作函数
        ...
        //     type: "opencheck"
        // })
        window.sessionStorage.setItem('ajgl_bh',record.wfbh);
        window.sessionStorage.setItem('webType','jycx');
        avalon.history.setHash('/zfsypsjglpt-zfda-ajgl-detail_gongan');
    }
})

avalon.component('ms-sgcl-list-jycx', {
    template: __inline('./zfsypsjglpt-zfda-sgcl-list-jycx.html'),
    defaults: {
        ...
        onInit() {
            ...
            menuServer.menu.then(menu => {
                ...
                avalon.each(func_list, function (k, v) {
                    switch (v) {
                        case "AUDIO_FUNCTION_ZFDA_JTWF_JYCX_CHECK":
