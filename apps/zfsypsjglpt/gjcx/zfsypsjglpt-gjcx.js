...
export const name = 'zfsypsjglpt-gjcx';
let vm = null;
...
avalon.component(name, {
    template: __inline('./zfsypsjglpt-gjcx.html'),
    defaults: {
        ...
        fetchOrgData() {
            ...
            getOrgAll().then((ret) => {
                if (ret.data.length == 0) {
                    showMessage('warn', vm.extra_class ? "Get your affiliate failure!" : '暂无所属部门，请于运维中心设置视图！');
                    vm.noOrgData = true;
                    return;
                }
                vm.noOrgData = false;
        ...
        handleSourceTypeChange(ev) {
            ...
            switch (ev.target.value) {
                ...
                case 'DEVICE_STORAGE_CAPACITANCE':
                    ...
                    break;
                case 'DEVICE_MOTIONLESS':
                    json.DEVICE_SOSType = 'DEVICE_MOTIONLESS';
                    break;
                case 'DEVICE_ONLINE':
            ...
            getTableData(json).then(ret => {
                ...
                // setTimeout(function(){
                    ...
                    $("[data-toggle='popover']").popoverX({
                        ...
                        //     var dep_orgCode = $(event.target).attr('data-origin-title');
                        ...
                        //         if(res.data=="") {
                        //             $(event.target).attr('data-origin-title', $(event.target).text());
                        //         }else {
                        //             $(event.target).attr('data-origin-title', res.data);
                        //         }
        ...
        gjgllook($index) {
            ...
            obj.type = this.type[item.type];;
            sessionStorage.setItem('gjgl-gjglcontrol', JSON.stringify(obj));
            let url = window.location.href;
            window.location.href = url.substring(0, url.lastIndexOf('/')) + "/zfsypsjglpt-gjcx-detail";
        }
    }
});
...
avalon.filters.changeData = function (str, arg) {
    let sosSource = {
        ...
        DEVICE_MOTIONLESS: vm.extra_class ? 'Equipment Static' : '设备静止',
    ...
    if (arg == "sosSource") {
        strs = typeof str == 'string' ? sosSource[str] : {
            'data-origin-title': sosSource[str['data-origin-title']],
            'data-toggle': 'popover'
        }; //处理插值表达式或者title
    } else if (arg == "status") {
        strs = typeof str == 'string' ? status[str] : {
            'data-origin-title': status[str['data-origin-title']],
            'data-toggle': 'popover'
        };
    } else if (arg == 'sosLevel') {
        strs = typeof str == "number" ? sosLevel[str] : {
            'data-origin-title': sosLevel[str['data-origin-title']],
            'data-toggle': 'popover'
        };
