...
require('./zfsypsjglpt-jdkp-fxccf-wkp_jiaojing.css');
export const name = 'zfsypsjglpt-jdkp-fxccf-wkp_jiaojing';
let storage = require('/services/storageService.js').ret;

let ajglMan = null;
let fxccf_ck_Man = null;
...
let ajgl_session = (function () {
    ...
    return {
        getAjbh: function () {
            xh = storage.getItem('zfsypsjglpt-jdkp-fxccf-current-fxccfData').xh;
            return xh;
        }
    };
})();
...
avalon.component(name, {
    template: __inline('./zfsypsjglpt-jdkp-fxccf-wkp_jiaojing.html'),
    defaults: {
        ...
        ajgl_back(e) {
            ...
            $('.popover').hide();
            Tools.clearPlayer('');
            fxccf_ck_Man.jckpStatus = false;
            fxccf_ck_Man.fzkpStatus = false;
            ...
            //路由跳转至查询页面
            let init_data = storage.getItem("zfsypsjglpt-jdkp-fxccf-current-fxccfData");
            if (init_data) {
                if (init_data.sjfxccfkp) {
                    avalon.history.setHash("/zfsypsjglpt-jdkp-sjfxccfkp");
                } else {
            ...
            storage.removeItem("zfsypsjglpt-jdkp-fxccf-current-fxccfData");
        ...
        toggleMt(e, z, x) {
            ...
            var type = fxccf_ck_Man.glmt_mark;
            var aim = null;
            fxccf_ck_Man[type + '_data'].forEach(function (val, key) {
        ...
        playFile(e) {
            ...
            // fxccf_ck_Man.checkLooking = '正在查看：' + name;
            ...
            if (name && orgNameStr) {
                fxccf_ck_Man.checkLooking = '正在查看：' + orgNameStr + ' - ' + userNmaeAndCode + ' - ' + name;
            } else {
                fxccf_ck_Man.checkLooking = '正在查看：' + name;
            }

            var mark = fxccf_ck_Man.glmt_mark;
            ...
            ajax({
                ...
                if (ret.code == 0) {
                    ...
                    if (ret.data.saveTime == '已过期') {
                        ...
                        fxccf_ck_Man.imgff = false;
                        fxccf_ck_Man.ocxPlayer = false;
        ...
        fxccf_titleShow_kp: '考评', //考评|考评结果|核查处理|考评核查结果
        ...
        cjxx(kpType) {
            ...
            this.kpType = kpType;
            this.cjxx_show = -1;
            fxccf_ck_Man.cjxxInfo.selectEva = [];
            tempUnselected = [];
            if (kpType == 'jckp') {
        ...
        cjxxInfo: avalon.define({
            ...
            handleChangePassed(e) {
                let _this = this;
                _this.fxccfPassed = e.target.value == 'Y' ? true : false;
            ...
            handleChange(e) {
                ...
                if (tempArrChecked) {
                    _this.fxccfPassed = false; //如果选择了一项考评为 是，则不通过考评；
                }
            ...
            submitComment() {
                ...
                if (fxccf_ck_Man.cjxxInfo.commentValue.length > 500) {
                    notification.warn({
                        message: '评语最多输入500个字',
                        title: '温馨提示'
                    });
                ...
                if (fxccf_ck_Man.cjxxInfo.selectEva.length == 0) {
                    notification.warn({
                ...
                // if (fxccf_ck_Man.cjxxInfo.selectCheck[0].check == 'none') {
                //     notification.warn({
                //         message: '请选择考评是否通过!',
                //         title: '温馨提示'
                //     });
                ...
                // if (fxccf_ck_Man.cjxxInfo.commentValue != "" || (tempUnselected.length >= 1)) { //考评项或 评语二选一
                if (fxccf_ck_Man.cjxxInfo.commentValue != "") { // 评语为必填项
        ...
        onInit(event) {
            ...
            fxccf_ck_Man.cjxx_show = '-2';
            //置空警情/媒体信息栏的初始值
            this.jbxxInfo.infomation = {};
            fxccf_ck_Man.cjxxInfo.selectCheck[0].check = 'none'; //初始化通过选项
            ...
            currentList.evaStatus = (storage.getItem("zfsypsjglpt-jdkp-fxccf-current-fxccfData").evaStatus) ? true : false;
            currentList.reviewStatus = (storage.getItem("zfsypsjglpt-jdkp-fxccf-current-fxccfData").reviewStatus) ? true : false;

            //请求警情信息并进行页面填充操作
            globalEvaList = {
                retData: []
            };
            fxccf_ck_Man.authority.SEARCH = true;

            Tools.searchCaseInfo(); //再次，解决ie和火狐执行顺序不一致bug，初始化基本信息和处警信息  

            fxccf_ck_Man.ocxPlayer = true;
                ...
                avalon.each(func_list, function (k, v) {
                    switch (v) {
                        ...
                        case "AUDIO_FUNCTION_JDKP_AJKP_JCKP":
                            fxccf_ck_Man.authority.JCKP = true;
                            break;
                        case "AUDIO_FUNCTION_JDKP_AJKP_FZKP":
                            fxccf_ck_Man.authority.FZKP = true;
                            break;
                        case "AUDIO_FUNCTION_JDKP_JTWFKP_FXCCF_KP":
                            fxccf_ck_Man.authority.JTWFKP_KP = true;
                            break;
                    }
                });
                // 无考评权限时选中第一个tab
                if (!fxccf_ck_Man.authority.JTWFKP_KP) {
                    fxccf_ck_Man.jbxx();
                }
            });
        },
        onReady() {
            ...
            $(window).bind('beforeunload', function (event) { //ie
                ...
                if (global.location.hash.indexOf("zfsypsjglpt-jdkp-fxccf") > -1)
                    avalon.history.setHash("/zfsypsjglpt-jdkp-fxccf_jiaojing");
            });
...
let confirmCont = avalon.define({
    ...
    confirmOk() {
        ...
        if (confirmVm.operation_type == 1) {
            let clickType = fxccf_ck_Man.kpType;
            let tempType = fxccf_ck_Man.kpType == 'jckp' ? "BASIC" : "LEGAL";
            ajax({
                url: '/gmvcs/audio/surveil/eva',
                ...
                data: {
                    ...
                    evaInfo: {
                        evaItems: fxccf_ck_Man.cjxxInfo.selectEva,
                        comment: fxccf_ck_Man.cjxxInfo.commentValue,
                        passed: fxccf_ck_Man.cjxxInfo.fxccfPassed //是否通过
                    }
                },
                cache: false
            }).then(ret => {
...
function markRedIcon() {
    ...
    let temp = document.getElementById("commentBox");
    if (fxccf_ck_Man.cjxxInfo.commentValue == "") {
...
let Tools = {
        ...
        ajax({
            ...
            if (ret.code == 0) {
                ...
                if (!ret.data) {
                    ...
                    fxccf_ck_Man.cj_data = [];
                    fxccf_ck_Man.jq_nodata = !Boolean(fxccf_ck_Man.jq_data.length);
                    fxccf_ck_Man.cj_nodata = !Boolean(fxccf_ck_Man.cj_data.length);
                ...
                //处理选中已经总共数量的显示
                // fxccf_ck_Man.jq_data = ret.data.handelerBaseFiles;
                fxccf_ck_Man.cj_data = ret.data.handelerBaseFiles;
                fxccf_ck_Man.jq_nodata = !Boolean(fxccf_ck_Man.jq_data.length);
                fxccf_ck_Man.cj_nodata = !Boolean(fxccf_ck_Man.cj_data.length);
                this.configGlmtNumber(); //关联媒体总数和选中个数
    ...
    clearPlayer: function (type) {
        var playing = type == '_dia' ? tjgl_form : fxccf_ck_Man;
    ...
    searchCaseInfo: function () {
        // Tools.searchMediaByJqbh();      
        ajax({
            url: '/gmvcs/audio/surveil/search/' + ajgl_session.getAjbh(),
            // url: '/api/jdkpFXCCF-hqxxxx',            
            method: 'get',
            data: '',
            cache: false
        }).then(ret => {
            ...
            if (ret.code == 0) {
                ...
                if (!ret.data) {
                    ...
                    var type = fxccf_ck_Man.glmt_mark;
                    fxccf_ck_Man.glmt_total = fxccf_ck_Man[type + '_data'] ? fxccf_ck_Man[type + '_data'].length : 0;
                    var ret_data = ret.data;
                    ret_data.xh; //简易程序违法数据编号-主键
                    ...
                    ret_data.files; //非现场处罚违违法数据关联到的媒体
                    fxccf_ck_Man.cj_data = ret_data.files; //简易程序违法数据关联到的媒体
                    if (fxccf_ck_Man.cj_data) {
                        fxccf_ck_Man.cj_data.forEach(function (val, key) {
                            val.checked = false;
                            val.importTime = formatDate(val.importTime);
                            val.startTime = formatDate(val.startTime);
                            val.duration = Tools.formatDuring(val.duration * 1000);
                        ...
                        fxccf_ck_Man.cj_nodata = true;
                    }
                    ret.data.wfsj = formatDate(ret.data.wfsj);
                    ret_data.evaInfo; //非现场处罚违违法数据考评信息
                    ret_data.reviewInfo; //非现场处罚违违法数据考评核查信息

                    //处理案件信息
                    fxccf_ck_Man.checkLooking = (ret.data.wfxwmc || '-') + '(' + (ret.data.jdsbh || '-') + ')';

                    fxccf_ck_Man.jbxxInfo.infomation = ret.data;
        ...
        //初始情况放在这,统一解决选中警情信息        
        fxccf_ck_Man.jbxx_clickClass = "cjxx-btn";
        fxccf_ck_Man.jckp_clickClass = "jbxx-btn";
        fxccf_ck_Man.jbxx_show = false;
        fxccf_ck_Man.cjxx_show = -1;
        fxccf_ck_Man.jckpStatus = true;
        fxccf_ck_Man.fzkpStatus = false;
        fxccf_ck_Man.kpType = 'jckp';

        if (currentList.evaStatus) {
            getEvaInfo("BASIC");
            fxccf_ck_Man.fxccf_titleShow_kp = '考评结果';
        } else {
            getPolicesituation();
            fxccf_ck_Man.fxccf_titleShow_kp = '考评';
        }


    },
    clickGlmt: function (type) {
        fxccf_ck_Man.glmt_name.forEach(function (val, key) {
            fxccf_ck_Man[val + '_clickClass'] = 'glmt_cj';
            fxccf_ck_Man['glmt_' + val + '_show'] = false;
        });
        fxccf_ck_Man[type + '_clickClass'] = 'glmt_jj';
        fxccf_ck_Man['glmt_' + type + '_show'] = true;
        fxccf_ck_Man.glmt_mark = type;
        Tools.configGlmtNumber();
    },
    configGlmtNumber: function () {
        var type = fxccf_ck_Man.glmt_mark;
        fxccf_ck_Man.glmt_total = fxccf_ck_Man[type + '_data'].length;
    },
    init: function () {
...
//已考评:获取非现场考评项列表[基层考评跟法制考评复用:传类型进去]
function getEvaInfo(type) {
    ajax({
        url: '/gmvcs/audio/surveil/eva/info/' + ajgl_session.getAjbh(),
        ...
        if (ret.code == 0) {
            let tempEvaList = [];
            fxccf_ck_Man.noneKP_cont.alreadyEvaLists = [];
            if (!ret.data) {

            } else {
                if (ret.data.comment) {
                    fxccf_ck_Man.noneKP_cont.comment = ret.data.comment;
                } else {
                    fxccf_ck_Man.noneKP_cont.comment = "无";
                }


                fxccf_ck_Man.noneKP_cont.result = (ret.data.passed ? '通过。原因：' : '不通过。原因：') + ret.data.result;
                fxccf_ck_Man.noneKP_cont.orgName = ret.data.orgName;
                fxccf_ck_Man.noneKP_cont.userName = ret.data.userName;
                fxccf_ck_Man.noneKP_cont.userCode = ret.data.userCode;
                fxccf_ck_Man.noneKP_cont.createTime = formatDate(ret.data.createTime);
...
function getSelectedItem(status) {
    switch (status) {
        ...
        case "fzkp":
            fxccf_ck_Man.jbxx_clickClass = "cjxx-btn";
            fxccf_ck_Man.fzkp_clickClass = 'jbxx-btn';
            fxccf_ck_Man.jbxx_show = false;
            fxccf_ck_Man.fzkpStatus = true;
            fxccf_ck_Man.kpType = 'fzkp';
            break;
    }
}
    ...
    ajax({
        ...
        if (ret.code == 0) {
            if (ret.data.length != 0) {
                ...
                fxccf_ck_Man.cjxxInfo.selectEva = selectEva;
                fxccf_ck_Man.cjxxInfo.evaLists = ret.data;
            ...
            //展示未考评
            fxccf_ck_Man.cjxxInfo.commentValue = "";
            fxccf_ck_Man.cjxx_show = 0;
        }
    });
}
...
let filePlayer = (function () {
    ...
    return {
        play: function (type, rid, dia) {
            ...
            States.playing = dia == '_dia' ? tjgl_form : fxccf_ck_Man;
            ajax({
